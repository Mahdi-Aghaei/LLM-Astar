...
class AStar:
    ...
    def searching(self, query, filepath='temp.png'):
        ...
        self.CLOSED = set()  # CLOSED set / VISITED order (set for faster lookups)
        ...
        while self.OPEN:
            ...
            self.CLOSED.add(s)  # Use set for faster membership check
        ...
        print(result)
        self.plot.animation(path, visited, True, "A* enhanced By Parsa", self.filepath)
        return result
    
    @staticmethod
    def _euclidean_distance_squared(p1, p2):
        # Use squared Euclidean distance for faster comparison
    ...
    def get_neighbor(self, s):
        ...
        # Filter out invalid neighbors in a single pass
    ...
    def is_valid(self, s):
        """
        بررسی می‌کند که آیا گره s در محدوده محیط و بدون برخورد است.
        :param s: گره (state)
        :return: True اگر گره مجاز باشد، False در غیر این صورت
        """
        # بررسی اینکه آیا گره خارج از مرزها است
        if not (0 <= s[0] < self.range_x[1] and 0 <= s[1] < self.range_y[1]):
            return False

        # بررسی برخورد با موانع افقی
        for horizontal in self.horizontal_barriers:
            if horizontal[0] <= s[1] <= horizontal[1] and s[0] == horizontal[2]:
                return False

        # بررسی برخورد با موانع عمودی
        for vertical in self.vertical_barriers:
    ...
    def heuristic(self, s):
        ...
        """
        return self._euclidean_distance_squared(s, self.s_goal)  # Use squared heuristic
